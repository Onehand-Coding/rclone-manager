...
**Recent Work:**
- 2026-07-31 — Fixed `rich.errors.LiveError: Only one live display may be active at once` crash in `serve-remote` with multiple remotes (or drive shared drive). Each serve thread called `console.status()` on the same shared Rich console; Rich only allows one Live per console. Moved the status to a single `with console.status(...)` on the main thread showing all served remotes (e.g. `Serving A, B...`) while threads just `console.print` (Rich coordinates prints with an active Live); threads no longer touch `status`. (`serve.py:88-108`)
