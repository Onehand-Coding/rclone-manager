...
def list_remote_directory_contents(remote_path):
    ...
    try:
        ...
        output = subprocess.check_output(["rclone", "lsf", "--dirs-only", remote_path]).decode("utf-8")
        dirs = [item.strip() for item in output.strip().split("\n") if item.strip()]

        output = subprocess.check_output(["rclone", "lsf", "--files-only", remote_path]).decode("utf-8")
        ...
        for item in files:
            ...
            try:
                size_output = subprocess.check_output(["rclone", "ls", "--max-depth", "1", f"{remote_path}{item}"]).decode("utf-8")
...
def main_app():
    ...
    with st.sidebar:
        ...
        if not st.session_state.authenticated:
            ...
            correct_username = os.environ.get("WEBUI_USERNAME", os.environ.get("USERNAME", "admin"))
            correct_password = os.environ.get("WEBUI_PASSWORD", os.environ.get("PASSWORD", "rclone"))
            ...
            if remotes:
                ...
                selected_option = st.selectbox("Select Location", remote_options,
                                             index=0 if st.session_state.current_remote is None else remote_options.index(st.session_state.current_remote) if st.session_state.current_remote in remote_options else 0)
                ...
                st.info("No rclone remotes found. Configure rclone remotes to access cloud storage.")
    ...
    with col1:
        ...
        if st.session_state.current_remote:
            ...
            current_display_path = st.session_state.remote_path if st.session_state.remote_path else f"{st.session_state.current_remote}:"
            st.text(f"Current: {current_display_path}")

            # Toggle for showing hidden files
            st.session_state.show_hidden = st.toggle("Show hidden files", value=st.session_state.show_hidden)
            ...
            contents = list_remote_directory_contents(st.session_state.remote_path or f"{st.session_state.current_remote}:")
        else:
        ...
        for item in contents:
            ...
            with cols[0]:
                ...
                if is_selected:
                    ...
                    if st.session_state.current_remote:
                        item_path = f"{st.session_state.remote_path.rstrip('/')}/{item['name']}" if st.session_state.remote_path else f"{st.session_state.current_remote}:{item['name']}"
                    else:
            ...
            with cols[1]:
                ...
                if item['is_dir']:
                    icon = "📁"
                    if st.button(f"{icon} {item['name']}", key=f"dir_{item['name']}", use_container_width=True):
                        if st.session_state.current_remote:
                            # Update remote path
                            new_path = f"{st.session_state.remote_path.rstrip('/')}/{item['name']}/" if st.session_state.remote_path else f"{st.session_state.current_remote}:{item['name']}/"
                    ...
                    if file_ext in ['.jpg', '.jpeg', '.png', '.gif', '.bmp', '.svg', '.webp']:
                        ...
                        icon = "📦"  # Archive file
                    elif file_ext in ['.py', '.js', '.ts', '.html', '.css', '.json', '.xml']:
                        icon = "💻"  # Code file
                    else:
    ...
    if st.session_state.current_remote:
        ...
        current_remote_path = st.session_state.remote_path or f"{st.session_state.current_remote}:"
        if current_remote_path.strip('/') != f"{st.session_state.current_remote}:".strip('/'):
            if st.button("📁 Go Up"):
                # Navigate up in remote path
                parent_path = '/'.join(current_remote_path.rstrip('/').split('/')[:-1]) + '/'
    ...
    with col2:
        ...
        if not st.session_state.current_remote:
            if st.button("📦 Download Selected as ZIP", disabled=len(st.session_state.selected_files) == 0):
        ...
        if not st.session_state.current_remote:
            ...
            if st.button("📥 Download Selected from Remote", disabled=len(st.session_state.selected_files) == 0):
